...
The mixture consists of $\ce{CaCO3}$ and $\ce{MgCO3}$.
...
\Sexpr{deltah_CaCO3} \mathrm{kJ/mol} = \Sexpr{cp_CaCO3}\mathrm{kJ/mol\cdot~K}(\Sexpr{T_kiln}-\Sexpr{T_0})\mathrm{K} \, ,
\end{equation}
%
where $cp_{\ce{CaCO3}}$ is given by \citet{NIST_2025}
The change in entropy is
\begin{equation}\label{eq:deltas_CaCO3}
\Sexpr{deltas_CaCO3} [kJ/mol\cdot~K] = \Sexpr{cp_CaCO3}[kJ/mol\cdot~K]\ln{\frac{\Sexpr{T_kiln}\mathrm{K}}{\Sexpr{T_0}\mathrm{K}}} \, .
\end{equation}

Therefore, the physical exergy of calcium carbonate ($\ce{CaCO3}$) is

\begin{equation}\label{eq:b_ph_CaCO3}
\Sexpr{b_ph_CaCO3}\mathrm{kJ/mol} = \Sexpr{deltah_CaCO3}\mathrm{kJ/mol} - (\Sexpr{T_0}\mathrm{K}) (\Sexpr{deltas_CaCO3}\mathrm{kJ/mol\cdot~K}) \, .
\end{equation}
...
\begin{align}
  b_{ch,\ce{CaCO3}} = \Sexpr{deltaG_MgCO3}\mathrm{kJ/mol}  &+ ((1 \left(\frac{\mathrm{mol_{\ce{Ca}}}}{\mathrm{mol_{\ce{CaCO3}}}}\right))\Sexpr{b_ch_Ca}\mathrm{kJ/mol})\notag\\
                                                          &+((1 \left(\frac{\mathrm{mol_{\ce{C}}}}{\mathrm{mol_{\ce{CaCO3}}}}\right))\Sexpr{b_ch_C}\mathrm{kJ/mol}\notag\\
                                                          &+((\frac{3}{2} \left(\frac{\mathrm{mol_{\ce{O2}}}}{\mathrm{mol_{\ce{CaCO3}}}}\right))\Sexpr{b_ch_O2}\mathrm{kJ/mol}\notag\\
                    = \Sexpr{b_ch_MgCO3}\mathrm{kJ/mol} \, . \notag
\end{align}
...
\begin{equation}\label{b_c_CaCO3}
  \Sexpr{b_c_CaCO3} [kJ/mol] = \Sexpr{RT0}[kJ/mol] \cdot~\ln(\Sexpr{y_CaCO3}[\frac{mol_{\ce{CaCO3}}}{mol_{limestone}}]) \, .
\end{equation}
...
\begin{equation}\label{eq:comminution_exergy_CaCO3}
  \Sexpr{b_com_CaCO3}[kJ/mol] = \frac{6[J/m^2](\Sexpr{Fr})}{\Sexpr{dM_CaCO3}[m]} (0.001[m^3/kg]) (0.001[kJ/J]) \, .
\end{equation}
%
...
\begin{itemize}
  \item With equations \ref{eq:physical_exergy}, \ref{eq:enthalpy}, and \ref{eq:entropy},
        ...
        % \begin{equation}\label{eq:deltah_MgCO3}
          b_{ph,\ce{MgCO3}} &= \Sexpr{cp_MgCO3}[kJ/mol\cdot~K](\Sexpr{T_kiln}-\Sexpr{T_0})[K] \\
                            &= \Sexpr{deltah_MgCO3}[kJ/mol] \, ,
        \end{align}
        %
        minus the change in entropy:
        \begin{equation}\label{deltas_MgC03}
          \Sexpr{deltas_MgCO3} \, \mathrm{kJ/mol\cdot~K} = \Sexpr{cp_MgCO3}[kJ/mol\cdot~K] \ln\left(\frac{\Sexpr{T_kiln}[K]}{\Sexpr{T_0}[K]}\right) \, ,
        \end{equation}
        %
        when entropy is multiplied by the ambient temperature:
        \begin{equation}\label{eq:b_ph_MgCO3}
          \Sexpr{b_ph_MgCO3}[kJ/mol] = \Sexpr{deltah_MgCO3}[kJ/mol] - \Sexpr{T_0}[K](\Sexpr{deltas_MgCO3}[kJ/mol\cdot~K]) \, .
        \end{equation}
        %
  ...
  \item The chemical exergy of $\ce{MgCO3}$ ($b_{ch,MgCO_3}$) is calculated with Eqn. \ref{eq:chemical_exergy_definition}.
        ...
        % \begin{equation}\label{eq:b_ch_MgCO3}
              b_{ch,\ce{MgCO3}} = \Delta{G_{f,\ce{MgCO3}}}                     &+ \sum_{j}{n_{j}b_{ch,j}} \tag{\ref{eq:chemical_exergy_definition}}\\
                                = \Sexpr{deltaG_MgCO3}\mathrm{kJ/mol}         &+ ((1 \left(\frac{\mathrm{mol_{\ce{Mg}}}}{\mathrm{mol_{\ce{MgCO3}}}}\right))\Sexpr{b_ch_Mg}\mathrm{kJ/mol})\notag\\
                                                                              &+((1 \left(\frac{\mathrm{mol_{\ce{C}}}}{\mathrm{mol_{\ce{MgCO3}}}}\right))\Sexpr{b_ch_C}\mathrm{kJ/mol}\notag\\
                                                                              &+((\frac{3}{2} \left(\frac{\mathrm{mol_{\ce{O2}}}}{\mathrm{mol_{\ce{MgCO3}}}}\right))\Sexpr{b_ch_O2}\mathrm{kJ/mol}\notag\\
                                = \Sexpr{b_ch_MgCO3}\mathrm{kJ/mol} \, . \notag
        \end{align}
  ...
  \item The concentration exergy of $\ce{MgCO3}$ ($b_{c,MgCO_3}$) is given by Eqn. \ref{eq:concentration_exergy_definition}
        ...
        \begin{equation}\label{eq:b_c_MgCO3}
          \Sexpr{b_c_MgCO3}[kJ/mol] = \Sexpr{RT0}[kJ/mol] \ln(\Sexpr{y_MgCO3}[\frac{mol_{\ce{MgCO3}}}{mol_{limestone}}]) \, .
        \end{equation}

  \item Comminution exergy
\end{itemize}
