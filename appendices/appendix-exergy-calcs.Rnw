...
\Sexpr{deltah_CaCO3} [kJ/mol] = \Sexpr{cp_CaCO3}[kJ/mol\cdot~K]\cdot~(\Sexpr{T_kiln}-\Sexpr{T_0})[K] \, ,
\end{equation}
%
and the change in entropy is
\begin{equation}\label{eq:deltas_CaCO3}
\Sexpr{deltas_CaCO3} [kJ/mol\cdot~K] = \Sexpr{cp_CaCO3}[kJ/mol\cdot~K]\ln{\frac{\Sexpr{T_kiln}[K]}{\Sexpr{T_0}[K]}} \, .
\end{equation}

Therefore, the physical exergy of calcium carbonate ($\ce{CaCO3}$) is

\begin{equation}\label{eq:b_ph_CaCO3}
\Sexpr{b_ph_CaCO3}[kJ/mol] = \Sexpr{deltah_CaCO3}[kJ/mol] - (\Sexpr{T_0}[K] \cdot~\Sexpr{deltas_CaCO3}[kJ/mol\cdot~K]) \, .
\end{equation}
...
\begin{equation}
  b_{ch,i} = \Delta{G_f,i} + \sum_{i}{n_{j}b_{ch,j}} \, . \tag{\ref{eq:chemical_exergy_definition}}
\end{equation}
%
For $\ce{CaCO3}$ the chemical exergy is
\begin{equation}\label{b_ch_CaCO3}
  \Sexpr{b_ch_CaCO3}[kJ/mol] = \Sexpr{deltaG_CaCO3}[kJ/mol] + ((1 [\frac{mol_{\ce{Ca}}}{mol_{\ce{CaCO3}}}]\cdot~\Sexpr{b_ch_Ca}[kJ/mol])+(1[\frac{mol_{\ce{C}}}{mol_{\ce{CaCO3}}}]\cdot~\Sexpr{b_ch_C}[kJ/mol])
  +(\frac{3}{2}[\frac{mol_{\ce{O2}}}{mol_{\ce{CaCO3}}}]\cdot~\Sexpr{b_ch_O2}[kJ/mol])) \, .
\end{equation}
***How do I wrap this to fit on the page?**-EES
...
\begin{itemize}
  \item With equations \ref{eq:physical_exergy}, \ref{eq:enthalpy}, and \ref{eq:entropy},
        ...
        \begin{equation}\label{eq:deltah_MgCO3}
          \Sexpr{deltah_MgCO3}[kJ/mol] = \Sexpr{cp_MgCO3}[kJ/mol\cdot~K](\Sexpr{T_kiln}-\Sexpr{T_0})[K] \, ,
        \end{equation}
        %
        minus the change in entropy:
        \begin{equation}\label{deltas_MgC03}
          \Sexpr{deltas_MgCO3}[kJ/mol\cdot~K] = \Sexpr{cp_MgCO3}[kJ/mol\cdot~K] \ln(\frac{\Sexpr{T_kiln}[K]}{\Sexpr{T_0}[K]}) \, ,
        \end{equation}
        %
  ...
  \item The chemical exergy of $\ce{MgCO3}$ ($b_{ch,MgCO_3}$) is calculated with Eqn. \ref{eq:chemical_exergy_definition}.
        \begin{equation}\label{eq:b_ch_MgCO3}
          \Sexpr{b_ch_MgCO3}[kJ/mol] = \Sexpr{deltaG_MgCO3}[kJ/mol] + ((1 [\frac{mol_{\ce{Mg}}}{mol_{\ce{MgCO3}}}]\cdot~\Sexpr{b_ch_Mg}[kJ/mol])
          +(1[\frac{mol_{\ce{C}}}{mol_{\ce{MgCO3}}}]\cdot~\Sexpr{b_ch_C}[kJ/mol])
          +(\frac{3}{2}[\frac{mol_{\ce{O2}}}{mol_{\ce{MgCO3}}}]\cdot~\Sexpr{b_ch_O2}[kJ/mol])) \, .
        \end{equation}
  ...
  \item The concentration exergy of $\ce{MgCO3}$ ($b_{c,MgCO_3}$) is given by Eqn. \ref{eq:concentration_exergy_definition}
        ...
        \begin{equation}\label{eq:b_c_MgCO3}
          \Sexpr{b_c_MgCO3}[kJ/mol] = \Sexpr{RT0}[kJ/mol]\cdot~\ln(\Sexpr{y_MgCO3}[\frac{mol_{\ce{MgCO3}}}{mol_{limestone}}]) \, .
        \end{equation}
