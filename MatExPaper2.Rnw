...
print_statepoint_table <- function(statepoint_table_name,
  ...
  colnames(table_df) <- col_header_latex

  xt <- xtable::xtable(table_df,
                       digits = digits,
                       caption = caption,
                       align = c("r", rep("c", ncol(table_df))))
...
You can also include a statepoint table as shown below.
...
You can also show a matrix as below.
...
<<ExampleMakeCell, echo=FALSE, results=tex>>=
# Sample data
df <- data.frame(
  A = 1:3,
  B = 4:6
...
colnames(xtab) <- c("Col A", "Col B")

# Custom column headers with line breaks using \makecell
headers <- c("& \\makecell{First \\\\ Column} & \\makecell{Second \\\\ Column} \\\\")

# Print with add.to.row to customize header row
print(xtab,
      include.colnames = FALSE,
      add.to.row = list(pos = list(0), command = headers),
      sanitize.colnames.function = identity)
@
