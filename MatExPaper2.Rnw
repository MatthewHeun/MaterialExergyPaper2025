...
The entropy of a mixture ($S_{mix}$)
is given by
...
$N$ is the number of total moles in a mixture,
...
the total material exergy of a compound ($i$) at a statepoint
is defined as
...
where $N_{mix,1}$ is the number of moles in a mixture at statepoint 1.


% \begin{equation}\label{eq:specific_exergy_of_mixture_definition1}
%   B_{mix} = T_0 S_{mix} = -R T_0 \sum_{i}{N_{i}{y_{i}} \ln{y_{i}}} \; ,
% \end{equation}
% %
