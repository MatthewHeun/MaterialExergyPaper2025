...
Chemical exergy ($b_{ch}$) is given by
...
is equivalent to the moles of the species.\\

\citet{Valero_2015} define concentration exergy of a compound as
...
A processes' efficiency
in a material conversion chain
varies depending on what information is desired.
...
\begin{itemize}
  ...
  \end{equation}
  %
  where $\eta_{x}$ is the efficiency of a process.

  In the case of the blast furnace,
  the Bejan method uses the material exergy ($b_{m,Fe_2 O_3}$)
  of hematite ore
  and compares it to the material exergy
  at the next statepoint.
  The energy exergy input is the work it takes
  to concentrate the hematite ore to pig iron.
...
%subsections for materials data
%subsections for energy data
