...
\label{sec: material exergy}
%++++++++++++++++++++++++++++++

****
I think much of this section needs to be revisited
with our new understanding of how to calculate
chemical exergy.
---MKH
****
...
defined by $T_0$, $P_0$, and a certain chemical composition.
...
For our purposes,
the reference environments are at $T_0$ and $P_0$,
and are characterized by a mixture of reference species.
When calculating exergy for ambient conditions
the thermo-mechanical exergy ($b_{th}$) is considered zero.
...
\begin{equation}\label{eq:general_material_exergy_definition}
  b_i = b_{ch} + b_c + b_{com}
\end{equation}

where $b_{ch}$ is the chemical exergy
and $b_{c}$ is the concentration exergy,
and $b_{com}$ is the comminution exergy
according to \citet{Valero_2015}.

%++++++++++++++++++++++++++++++
\subsubsection{Chemical Exergy Calculation}
\label{sssec: chemical exergy calculation}
%++++++++++++++++++++++++++++++

Chemical exergy ($b_{ch,i}$) is the minimum work needed to form a compound ($i$) or element ($j$).
Concentration exergy ($b_{c,i}$) is the minimum work done to concentration a species
from a pure substance to a different molar concentration ($y_i$).
...
\label{sssec: concentration exergy calculation}
%++++++++++++++++++++++++++++++

Concentration exergy ($b_c$) is the the work required
to change the molar fraction ($y$) of a species.
The definition of concentration exergy is derived from the entropy of mixing.

The entropy of a mixture ($S_m$)
is given by
...
and $y_i$ is the molar fraction of a species in a mixture.
The number of moles in a mixture is determined by
the mass composition of the stream and
each component's molecular weight,
which we take from \citet{Fisher_2025}.

The molar fraction of a species always refers back to the molar fraction
of the species as a pure substance.
The molar fraction of a pure substance ($y_{i,0}$) is equal to one
because the number of total moles in a mixture
is equivalent to the number of moles of the species.\\


\citet{Valero_2025} defines concentration exergy of a compound as
...
\label{sec: material exergy calculation}
%++++++++++++++++++++++++++++++

With equations \ref{eq:concentration_exergy_definition}
and \ref{eq:chemical_exergy_definition}
the total material exergy of a compound ($i$) within a mixture ($m$)
...
The total exergy of energy ($X$)
of a statepoints
is determined by the type of energy utilized.
**** Let's be more concise here. ---MKH ****
\st{A constant, denoted by ($\phi$),
is multiplied by the energy input
to calculate the exergy.}
Energy inputs are multiplied by an exergy-to-energy ratio ($\phi$)
to obtain exergy inputs.
The $\phi$ constant is a function of the type of energy used.
For example, natural gas has $\phi = 1.04$
while coking coal has $\phi = 1.05$.
**** Be more concise. ---MKH ****
\st{Further information about the phi constants used
may be found in Appendix~X.
Phi constants and examples for certain fuels come from} \citep{Kotas_1995}.
We take $\phi$ values from \citet{Kotas_1995}.
See Appendix~X.
...
**** be more precise. ``input to a process?'' ---MKH ****
...
****
Does the ``i`` subscript above mean the same thing it meant earlier in the paper?
---MKH
****
...
\begin{itemize}
  ...
  See Figure~\ref{fig:efficiencymethods}.
...
Physical Supply Use Tables (PSUTs) matrices
...
and end-use efficiencies for material, energy, and exergy.

The PSUT framework is applied to conversion chains.
Once the material and energy conversion chains are modelled,
both chains may be converted to exergy chains.

**** Redo the next two paragraphs as shown below. ---MKH ****
Figure 1 from ~\citep{Aramendia_2022} is the Physical Supply Use Tables framework
applied to energy, material, and exergy conversion chains in this paper.
The main four matrices the conversion chains are applied to are the Resources, Make, Use, and Final demand matrices.
Table 1 from ~\citep{Aramendia_2022} explains the rows and columns for each matrix.

The Resource matrix ($R$) is an exogenous resource-by-resource matrix.
The Make matrix ($V$) is an industry-by-product matrix.
The Use matrix ($U$) is a product-by-industry matrix.
The Final demand matrix ($Y$) is a product-by-sector matrix.
...
\begin{table}[H]
\caption{Nomenclature tabl.\label{tab5}}
