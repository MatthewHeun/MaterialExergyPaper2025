...
to calculate the exergy ($X$).

\begin{equation}\label{eq:energy_exergy_definition}
  X_{i} = \phi E_i
\end{equation}
where $E_i$ is the energy of a species,
such as natural gas.

The exergy of energy used
...
Phi constants and examples for certain fuels come from \citet{Kotas_1995}.
