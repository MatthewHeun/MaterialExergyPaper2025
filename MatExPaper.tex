...
The number of moles in a mixture is determined by the mass composition of the stream and each components molecular weight which come from Fisher Scientific \citep{Fisher_2025}.
...
$\delta{G_{f}}$ values come from \citep{Dean_1979}
...
Final demand for energy and material products can be tracked to primary resources
with PSUT matrices.
End-use efficiencies can be seen from material, energy, and exergy PSUT matrices.

Material and energy conversion chains are converted
to an exergy conversion chain which allows us to create exergy PSUT matrices.
...
The Resource matrix has exogenous resources in the rows and resources in the columns.
The Make matrix has industries in the rows and products in the columns.
The Use matrix has products in the rows and industries in the columns.
The Final demand matrix has products in the rows and sectors in the columns.
...
\citep{Ore_2025}) and \citep{Uni_Cape_Town_2002}.
Additional energy information comes from \citep{Jamison_2015}.
...
Coal production compositions and energy inputs come from \citep{Matsuhashi_2012},
\citep{Zhang_2019}, \citep{Kotas_1995}, and various webpages.
Limestone production compositions and energy inputs come from \citet{Saidur_2011},
\citep{Eriksson_2014}, and
\citep{Satyendra_2013}.
...
% \addbibresource{ReferencesUpdate.bib} %Import the bibliography file
